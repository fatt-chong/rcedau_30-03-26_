...
//if($_GET['bandera'] == 'aprobarIndicacion' || $_GET['bandera'] == 'generarEsquema'){ 
    // $miarray                                                    = "ldiaz";
if($_SESSION['MM_Username'.SessionName] != "" ){
    $miarray                                                    = $_SESSION['MM_Username'.SessionName];
}else{
    $miarray                                                    = base64_decode($_GET["k"]);
}
    ...
    $rs_crs = mysqli_query($conn, $sql);
    // $rs_crs                                                     = mysql_query($sql) or die(mysql_error());
    $account = mysqli_fetch_assoc($rs_crs);
    ...
    $rs_AplicacionesxUsuarios = mysqli_query($conn, $sqlrol);


    $i = 0;
    $permiso = array(); // Asegúrate de inicializar el arreglo

    // Verifica si la consulta devuelve resultados
    if ($rs_AplicacionesxUsuarios) {
        // Recorre los resultados de la consulta
        while ($row = mysqli_fetch_assoc($rs_AplicacionesxUsuarios)) {
            // Almacena el valor de 'rol_idrol' en el arreglo $permiso
            $permiso[$i++] = $row['rol_idrol'];
        }
    } else {
    ...
    if($account['rutusuario'] > 0){
        ...
        // $_SESSION['pyxi_SERVER'.SessionName]        = '10.6.18.95';
        // $_SESSION['BD_SERVER'.SessionName]          ='10.6.21.26';
        // $_SESSION['tiempoInactividad'.SessionName]  = '10';
        // $_SESSION['MM_Servicio_activo'.SessionName] = '';

        // $_SESSION['ContadorSession' . SessionName]          = 2;
        // $_SESSION['idUsuario'.SessionName]        = $account['idusuario'];
        // $_SESSION['MM_Servicio' . SessionName]            = $account['idservicio'];
    }
...
<html lang="en">
    <head>
        ...
        <script type="text/javascript" src="<?=RAIZ?>/controllers/main.js?v=<?=date('H.i.s')?>1"></script>
        ...
        <script type="text/javascript" src="<?=PATH?>/assets/libs/highcharts/highcharts.js"></script>
        <script type="text/javascript" src="<?=PATH?>/assets/libs/highcharts/jquery.highchartTable-min.js"></script>
        ...
        <script type="text/javascript" src="<?=PATH?>/assets/libs/Jquery.print/jQuery.print.min.js"></script>
        <!-- <script src="https://cdnjs.cloudflare.com/ajax/libs/Jquery.print/jQuery.print.min.js"></script> -->

        <script type="text/javascript" src="<?=PATH?>/controllers/client/identificacion/identificador.js?v=<?=$version;?>"></script>
        ...
        <!-- <script src="http://10.6.21.36:4001/socket.io/socket.io.min.js"></script> -->
        <script type="text/javascript" src="../estandar/jquery.timeEntry/jquery.timeentry.js"></script>
        <script src="<?=RAIZ?>/assets/js/select2.min.js"></script>
        <link href="<?=RAIZ?>/assets/js//select2.min.css" rel="stylesheet">
     <style type="text/css">
         ::-webkit-scrollbar {
            cursor: pointer;
             width: 8px !important; 
    ...
    <?php 
// $file = 'C:/inetpub/wwwroot/php8site/test.txt';
// $content = "Prueba de escritura en el archivo.";
...
?>
    <body >
        ...
        <div id="wrapper" style="overflow-x: clip;">
            ...
            <div id="page-wrapper" class="py-2 "> 
