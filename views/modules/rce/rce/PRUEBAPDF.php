		...
		<tr>
	        <td class="enoform">
	        	<table class="enoformSin " cellspacing="2" border="0" width="100%">
					...
					<tr>
						<td width="50%">
							<strong >Fecha y Hora en que se entregó la información médica'.$arrayUsuariosIndicaciones[0] .'</strong>
						</td>
						<td width="50%">
					...
					<tr>
						<td width="50%">
							 '.$rsobtenerAcompaniante[0]['fechaEntregaInformacionMedica'].' '.$rsobtenerAcompaniante[0]['horaEntregaInformacionMedica'].'
						</td>
						<td width="50%">
...
if (is_array($registroViolencia) && count($registroViolencia) > 0 ) {
	...
	for ( $i = 0; $i < count($arrayUsuariosIndicaciones); $i++) {
		...
		if($file_headers_usuarioIndicaciones[0] == 'HTTP/1.1 200 OK') {
			...
			// 			<td>
			// 				<img  src="'.FirmaPDF.'/firmaDigital/medicos/'.$resultadoUsuarioIndicaciones[0]['PROcodigo'].'.png" style="width:150px; height:35px;">
			// 			</td>
			// 		</tr>
			// 		<tr style="text-align:center;">
			// 			<td>
